...
related_epic: EPIC-20260721-001-email-agent-port
---
...
YouTube 爬邮箱获客 → 用自己的 SMTP/IMAP 邮件谈价 → WhatsApp **协议桥**在桌面收发（人发，AI 翻译/建议）→ 每位客户的来源、报价、合作状态被系统记住，AI 通过 **只读查询** 获取客户上下文。

> **email-agent 迁入**（工作流、收件箱、待发人审、统计等）见 [EPIC-20260721-001](../changes/2026/07/epic-20260721-001-email-agent-port.md) 与 [分支手册](email-agent-port-branches.md)。KOL/Gbyte 对接暂缓。
...
6. WhatsApp **Baileys 协议桥**消息进入桌面；AI 翻译并建议回复（人点发送）
...
| **Epic：email-agent 迁入** | `changes/2026/07/epic-20260721-001-email-agent-port.md` | email-agent 能力重写、Phase 0~6 | 规划迁入工作前 |
| **分支手册** | `roadmaps/email-agent-port-branches.md` | 各 Phase 的 `pnpm branch:create` 命令 | 开分支前 |
| **Domain：Workflow** | `domains/workflow/README.md` | 建联阶段、话术、流程规则 | 改工作流前 |
| **Domain：Analytics** | `domains/analytics/README.md` | 统计、报表、开信追踪 | 改数据分析前 |
| **Domain：Schedule** | `domains/schedule/README.md` | 合作排期 | 改排期前 |
| **Domain：Alert** | `domains/alert/README.md` | 预警关键词 | 改预警前 |
| **Domain：KOL** | `domains/kol/README.md` | **暂缓**；仅占位 | KOL 迁入前 |
...
| **M5** WhatsApp 辅助 | proposed | **Baileys Worker** 桌面收发、翻译、建议 | CHG-020, **ADR-0006** | S6, **S6b** |
