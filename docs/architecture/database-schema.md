...
| `outreach_stage` | `no_stage`, `stage1`~`stage7`, `s_bounce`, `archived`, `archive_rejected`, `archive_budget_mismatch`, `archive_model_mismatch`, `archive_invalid_channel`, `archive_delayed_followup`, `archive_low_quality`, `archive_manual_pause` |
...
| `background_job.job_type` | `ocr`, `mail_send`, `imap_sync`, `mail_body_fetch`, `wa_protocol_sync`, `crawler_email_enrich`, `crawler_batch`, `pending_mail_notify`（预留） |
...
CREATE INDEX idx_customer_outreach ON customer(outreach_stage);
...
| Alert | `alert_keyword` |
| Schedule | `schedule_entry`, `schedule_product` |
| Analytics | `report_snapshot` |
...
## 10. email-agent 扩展表（规划，EPIC-20260721-001）

以下表在 MVP 基础表之上增量添加；**KOL 表见 [kol domain](../managed/domains/kol/README.md)（暂缓，本期无 migration）**。
...
| `alert_keyword` | Alert | 预警关键词 |
| `schedule_entry`, `schedule_product` | Schedule | 排期 |
| `report_snapshot` | Analytics | 报表快照 |

`customer.outreach_stage` 列见 §5.1 Customer（与 `lifecycle_status` 并存：前者为 email-agent 细粒度建联阶段）。

实施批次见 [email-agent-port-branches.md](../managed/roadmaps/email-agent-port-branches.md)。
...
| [CHG-033](../managed/changes/2026/07/chg-20260721-033-email-agent-docs-realignment.md) | email-agent 文档对齐 |
