...
/// 使用 Rust LLM 将纯文本邮件生成 HTML。
#[tauri::command]
pub async fn mail_generate_html(
    state: tauri::State<'_, AppState>,
    request: MailIpcGenerateHtmlRequest,
) -> Result<MailIpcGenerateHtmlResponse, String> {
    let client = stored_llm_client(state.inner()).await?;
    let generated = GenerateMailHtml::execute(&client, &request.text).await?;
    Ok(MailIpcGenerateHtmlResponse {
        ok: true,
        html: generated.html,
