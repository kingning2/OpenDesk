...
pub fn device_id_from_cookie(cookie_str: &str) -> Option<String> {
    super::cookies::device_id(&super::cookies::parse_credential(cookie_str))
}

/// 解析浏览器导出的 Cookie 凭据为键值表。
///
/// 兼容 `k=v; ...`、cookies JSON 数组、快照 JSON（与 [`super::cookies::parse_credential`] 一致）。
///
/// 作者：Xiaoman
/// 创建时间：2026-08-21
///
...
pub fn parse_cookies(cookie_str: &str) -> HashMap<String, String> {
    let header = super::cookies::credential_to_cookie_header(cookie_str);
    header
        .split(';')
        .filter_map(|part| {
            let part = part.trim();
