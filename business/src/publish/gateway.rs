...
pub trait PublishGateway: Send + Sync {
    /// 校验账号存在且有 Cookie。
    fn account_cookie(&self, user_id: i64, account_id: &str) -> OpenDeskResult<Option<String>>;
    ...
    /// 创建发布日志，返回日志 id。
    fn create_log(&self, entry: &PublishLogEntry, status: &str) -> OpenDeskResult<i64>;
    ...
    /// 发布成功后同步账号商品。
    async fn sync_account_items(&self, account_id: &str, cookie: &str) -> OpenDeskResult<SyncInfo>;
}
