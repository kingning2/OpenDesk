//! 1688 关键词搜索 — Sidecar Camoufox 指纹浏览器 + MTOP 拦截。
//!
//! 作者：Xiaoman
//! 创建时间：2026-08-22
...
pub struct Ali1688SearchRequest {
    pub owner_id: i64,
    pub account_id: String,
    pub keyword: String,
    #[serde(default)]
    pub max_results: Option<i64>,
    /// 有头指纹浏览器（默认 true，便于过滑块）。
...
/// 1688 搜索 IPC 出参（与 sidecar 响应对齐）。
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct Ali1688SearchResponse {
    pub ok: bool,
    pub status: String,
    pub keyword: String,
    pub total: i64,
...
/// 1688 关键词搜索（Camoufox 指纹浏览器）。
#[tauri::command]
pub async fn ali1688_search(
    state: State<'_, AppState>,
    account_handle: State<'_, AccountHandle>,
    request: Ali1688SearchRequest,
) -> common::DingDaResult<IpcResponse<Ali1688SearchResponse>> {
    state
        .license
        .ensure_licensed()
        .await
    ...
    if platform != "ali1688" {
        return Err(common::DingDaError::validation("仅支持 1688 账号搜索"));
    }
    ...
    let sidecar_request = ChannelSidecarSearchRequest {
        ...
        trace_id: Some(format!("ali1688-search-{}", request.account_id)),
    };
    ...
    Ok(IpcResponse::ok(Ali1688SearchResponse {
