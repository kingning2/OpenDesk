...
pub fn notification_channel_list(
    ...
    owner_id: i64,
) -> common::OpenDeskResult<Vec<NotificationChannel>> {
    let service = NotificationService::new(state.store.as_ref());
    service.list_channels(owner_id)
}
...
) -> common::OpenDeskResult<NotificationChannel> {
    let service = NotificationService::new(state.store.as_ref());
    service
...
pub fn notification_channel_test(
    ...
    let service = NotificationService::new(state.store.as_ref());
    service.test_channel(request.owner_id, request.channel_id)
}
...
pub fn notification_list(
    state: State<'_, NotificationHandle>,
    owner_id: i64,
) -> common::OpenDeskResult<Vec<MessageNotification>> {
    let service = NotificationService::new(state.store.as_ref());
    service.list_notifications(owner_id)
}
...
) -> common::OpenDeskResult<MessageNotification> {
    let service = NotificationService::new(state.store.as_ref());
    service.set_notification(
        request.owner_id,
        &request.account_id,
        request.channel_id,
        request.enabled,
