...
pub fn log_write(message: String, level: Option<String>) -> IpcResponse<()> {
    ...
    match level {
        "ERROR" => error!(target: "dingda.lifecycle", "{message}"),
        "WARN" => warn!(target: "dingda.lifecycle", "{message}"),
        "DEBUG" | "TRACE" => debug!(target: "dingda.lifecycle", "{message}"),
        _ => info!(target: "dingda.lifecycle", "{message}"),
    }
    IpcResponse::ok(())
}
