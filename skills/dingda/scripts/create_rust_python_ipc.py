...
def _schema(feature: str, action: str, direction: str) -> dict:
    return {
        ...
        "$id": f"dingda://{feature}/sidecar/{action}.{direction}/v1",
...
logger = logging.getLogger("dingda.sidecar.{feature}")
...
def main() -> int:
    ...
    example_dir = ROOT / "skills" / "dingda" / "examples" / "rust-python" / route_key
    write_text(
