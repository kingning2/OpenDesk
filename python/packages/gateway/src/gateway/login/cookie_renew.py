...
对齐 xianyu-auto-reply：稳态不常驻浏览器；仅续期时有头打开页面，
导出 Cookie 后关闭。验证码需用户在弹出窗口完成。
...
async def renew_cookies(
    ...
    *,
    ...
    try:
        ...
        logger.info(
            "开始浏览器续期 account=%s headless=%s url=%s",
            account_id,
            headless,
            target[:120],
        ...
        while time.monotonic() < deadline:
            ...
            if _looks_logged_in(url, exported, config.login_cookie_name):
                if "punish" in url.lower() or "captcha" in url.lower():
                    last_detail = "仍在验证码页，请在弹出的浏览器窗口完成验证"
        ...
        # 让事件循环收尾，避免 Windows 上 Chromium 句柄残留
